...
const Settings: React.FC<SettingsProps> = ({ canInstall, onInstall, setActiveTab }) => {
    const { user, userData, isLocalUser, updateLocalProfile } = useAuth();
                    ...
                    <motion.div variants={sectionVariants} className="bg-brand-surface/40 p-6 md:p-8 rounded-3xl border border-brand-border/50 shadow-xl backdrop-blur-sm flex flex-col">
                        ...
                        <div className="flex-1 space-y-6">
                            <div className="flex items-start gap-4">
                                <div className={`p-3 rounded-xl mt-1 ${user ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
                                    {user ? <Check size={20} /> : <AlertCircle size={20} />}
                                </div>
                                <div>
                                    <h4 className="font-bold text-brand-text mb-1">Cloud Sync Integration</h4>
                                    <p className="text-brand-text-secondary text-sm font-light leading-relaxed">
                                        {user 
                                            ? "Connected and ready to sync. Your data is securely backed up." 
                                            : "Create an account to enable secure cloud backups."}
                                    </p>
                                </div>
                            </div>
                            ...
                            {user ? (
                                ...
                                <div className="p-4 rounded-2xl bg-brand-primary/5 border border-brand-primary/10 flex flex-col gap-3">
                                    <div>
                                        ...
                                        <p className="text-[10px] text-brand-text-secondary leading-relaxed">Securely store your custom variables, preferences, and calculations by signing in.</p>
                                    </div>
                                </div>
                            )}
