                        ...
                        >
                            ...
                            {!user && (
                                <motion.div 
                                    ...
                                    className="mt-24 rounded-[3rem] bg-gradient-to-br from-brand-surface to-brand-bg border border-brand-border/30 overflow-hidden relative shadow-2xl group/repo"
                                >
                                    {/* Cosmic Glare Effect */}
                                    <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-l from-brand-primary/10 to-transparent opacity-0 group-hover/repo:opacity-100 transition-opacity duration-1000 z-0 pointer-events-none" />
                                    
                                    <div className="p-12 md:p-16 flex flex-col md:flex-row items-center justify-between gap-16 relative z-10">
                                        <div className="space-y-8 max-w-2xl relative z-10 w-full">
                                            <div className="inline-flex items-center gap-2 px-4 py-2 bg-brand-primary/10 text-brand-primary rounded-full text-xs font-black uppercase tracking-widest ring-1 ring-brand-primary/20 shadow-sm backdrop-blur-sm">
                                                <GraduationCap size={14} /> Scholastic Identity Core
                                            </div>
                                            
                                            <div className="space-y-4">
                                                <h4 className="font-black text-brand-text text-5xl md:text-6xl leading-[1.1] tracking-tight">
                                                    Unlock the<br/>
                                                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-brand-primary to-brand-accent animate-pulse">Scholastic Repository</span>
                                                </h4>
                                                <p className="text-brand-text-secondary text-xl font-light leading-relaxed max-w-lg">
                                                    Elevate your academic workflow. Preserve derivation logs, synchronize AI tutor memories globally, and unlock deep insights.
                                                </p>
                                            </div>
                                            
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-4">
                                                {[
                                                    { title: 'Global Sync', desc: 'Secure history across all devices' },
                                                    { title: 'AI Continuity', desc: 'Smarter tutoring with context' },
                                                    { title: 'Deep Analytics', desc: 'Track your growth metrically' },
                                                    { title: 'Priority Access', desc: 'Unlimited API bandwidth' }
                                                ].map((benefit, i) => (
                                                    <div key={benefit.title} className="flex items-start gap-3 bg-brand-surface/30 p-4 rounded-2xl border border-brand-border/30 hover:bg-brand-surface/50 transition-colors">
                                                        <div className="w-8 h-8 rounded-full bg-brand-primary/10 flex items-center justify-center shrink-0">
                                                            <div className="w-2 h-2 rounded-full bg-brand-primary animate-pulse" style={{ animationDelay: `${i * 0.2}s` }} />
                                                        </div>
                                                        <div>
                                                            <div className="text-sm font-bold text-brand-text">{benefit.title}</div>
                                                            <div className="text-xs text-brand-text-secondary mt-1">{benefit.desc}</div>
                                                        </div>
                                                    </div>
                                                ))}
                                            </div>
                                        ...
                                        <div className="w-full md:w-auto relative z-10 shrink-0 flex flex-col items-center justify-center gap-6">
                                            <div className="relative">
                                                <div className="absolute inset-0 bg-brand-primary/20 blur-[50px] rounded-full scale-150" />
                                                <button 
                                                    onClick={onLoginClick}
                                                    className="relative w-full md:w-auto px-16 py-8 bg-brand-text text-brand-bg rounded-[2.5rem] font-black text-sm uppercase tracking-[0.25em] hover:scale-105 active:scale-95 transition-all shadow-2xl flex flex-col items-center justify-center gap-2 overflow-hidden group/btn ring-4 ring-brand-text/10"
                                                >
                                                    <span className="relative z-10 flex items-center gap-3">
                                                        Activate Credentials <ArrowRight size={18} className="group-hover/btn:translate-x-1 transition-transform" />
                                                    </span>
                                                    <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/20 to-transparent -translate-x-full group-hover/btn:translate-x-full transition-transform duration-700" />
                                                </button>
                                            </div>
                                            <div className="flex items-center gap-2 text-brand-text-secondary text-xs uppercase tracking-widest font-black">
                                                <CheckCircle2 size={12} className="text-brand-accent" /> SSO Powered
                                            </div>
                                        </div>
                                    </div>
                                </motion.div>
