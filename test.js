...
This typically indicates that your device does not have a healthy Internet connection at the moment. The client will operate in offline mode until it is able to successfully connect to the backend.`;
  ...
  var firebase_applet_config_default = {
    projectId: "quantum-calculator-9c25e",
    appId: "1:933464056635:web:1fb63756ddc014bcbf8d1e",
    apiKey: "AIzaSyAkjRcLmH2ecduDTpYZSZ73QZO9SkfgtrA",
    authDomain: "quantum-calculator-9c25e.firebaseapp.com",
    firestoreDatabaseId: "ai-studio-9c9042da-24a3-42b8-a568-fd417e242625",
    storageBucket: "quantum-calculator-9c25e.firebasestorage.app",
    messagingSenderId: "933464056635",
    measurementId: ""
  };
  ...
  var dbId = firebase_applet_config_default.firestoreDatabaseId || "(default)";
  console.log(`[Firebase] Initializing Firestore with Database ID: ${dbId}`);
  var db = initializeFirestore(app, {
    experimentalForceLongPolling: true
  }, dbId);
  var supportedPersistence = [inMemoryPersistence];
  try {
  ...
  function handleFirestoreError(error, operationType, path) {
    ...
    const isPermissionError = errMsg.includes("Missing or insufficient permissions") || errMsg.includes("permission-denied") || error?.code === "permission-denied" || error?.code === "firestore/permission-denied";
    const errInfo = {
      error: errMsg,
      authInfo: {
  ...
  var AuthContext = (0, import_react.createContext)({
    ...
    loading: true,
    error: null,
    signInWithGoogle: async (_useRedirect) => {
    },
    signUpWithEmail: async (_email, _password, _displayName) => {
    },
  ...
  var AuthProvider = ({ children }) => {
    ...
    const [totalScholars, setTotalScholars] = (0, import_react.useState)(0);
    ...
    const unsubscribeUserDocRef = (0, import_react.useRef)(null);
    const unsubscribeStatsRef = (0, import_react.useRef)(null);
    ...
    (0, import_react.useEffect)(() => {
      ...
      const unsubscribeAuth = onAuthStateChanged(auth, async (currentUser) => {
        ...
        if (currentUser) {
          ...
          unsubscribeUserDocRef.current = onSnapshot(userRef, async (docSnap) => {
            if (docSnap.exists()) {
              ...
              try {
                ...
                const statsRef2 = doc(db, "stats", "globals");
                const statsSnap = await getDoc(statsRef2);
                if (statsSnap.exists()) {
                  await updateDoc(statsRef2, {
                    totalScholars: increment(1),
                    lastUpdated: serverTimestamp()
                  });
                } else {
                  await setDoc(statsRef2, {
                    totalScholars: 1,
                    lastUpdated: serverTimestamp()
                  });
                }
                ...
                handleFirestoreError(err, "write" /* WRITE */, `users/${currentUser.uid}`);
              }
            }
          }, (err) => {
            setLoading(false);
            setError(`Database connection failed: ${err.message}`);
            handleFirestoreError(err, "get" /* GET */, `users/${currentUser.uid}`);
          });
        } else {
          setUserData(null);
          setAccessToken(null);
    ...
    return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(AuthContext.Provider, { value: { user, userData, totalScholars, accessToken, loading, error, signInWithGoogle, signUpWithEmail, signInWithEmail, resetPassword, logout, clearError, signInSimulated }, children });
  };
})();
